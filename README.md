...
- `code/main.asm` is only the important code for the game, and contains the mainloop and vblank routines.

## Running the easy way
1. Run `build.bat` or `./build.sh` (double click in file explorer or run in terminal, I don't care)
2. Run `run.bat` or`./run.sh` in the same way
...
**To build**:
    ...
    - Or run `tools/asm6.exe ./code.asm ./code.nes`
